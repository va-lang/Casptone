						...
						<table class="table table-condensed table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="">Alert Of Information</th>
							...
							<tbody>
								<?php 
								$i = 1;
								$verification = $conn->query("SELECT verification.*, complaints.statusincident,complaints.address, complaints.tracking FROM `verification` inner join complaints on complaints.id=verification.complaint_id  order by id asc ");
								while($row=$verification->fetch_assoc()):
								?>
								<tr>
									<td class="text-center"><?php echo $i++ ?></td>
									<td>
										<p> <b><?php echo ucwords($row['statusincident']) ?></b></p>
									</td>
									<td class="">
										 <!-- <p>Contact #: <b><?php echo $row['contact'] ?></b></p>
										 <p>Address: </b></p> -->
										 <p><small><i><b><?php echo $row['tracking'] ?></i></small></p>
									</td>
									...
									<?php endif; ?>
										<button class="btn btn-sm btn-outline-primary view_complaints" type="button" data-id="<?php echo $row['complaint_id'] ?>"  data-name="<?php echo $row['id'] ?>">View Complaints</button>
									</td>
